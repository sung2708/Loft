...
const copies: LegalPageCopies = {
  vi: {
    ...
    notice: "Mingly không ghi hình phòng. Hiện Mingly không cung cấp kết nối Spotify hoặc OAuth cho dịch vụ âm nhạc. Bạn kiểm soát micro, camera, chia sẻ màn hình và trình phát YouTube từ trình duyệt.",
    sections: [
      ...
      { title: "Dịch vụ bên thứ ba và nội dung nhúng", paragraphs: ["Mingly sử dụng nhà cung cấp xác thực, cơ sở dữ liệu và truyền thông thời gian thực để vận hành dịch vụ. YouTube chỉ được dùng khi bạn chủ động tìm kiếm, thêm hoặc phát nội dung; trình phát nhúng của YouTube có thể áp dụng điều khoản và công nghệ của YouTube.", "Chúng tôi không bán dữ liệu cá nhân và không đưa token đăng nhập vào phòng hoặc trình duyệt của người khác."] },
      { title: "Lưu giữ, bảo mật và lựa chọn của bạn", paragraphs: ["Dữ liệu phòng và tin nhắn được lưu trong thời gian cần thiết để vận hành, bảo mật và xử lý yêu cầu hợp lệ. Chúng tôi áp dụng kiểm soát quyền truy cập ở máy chủ; không hiển thị mật khẩu phòng, token đăng nhập hoặc khóa dịch vụ cho người dùng khác.", "Mingly không thu thập hoặc lưu token Spotify hay token OAuth của dịch vụ âm nhạc. Bạn có thể rời phòng, tắt quyền thiết bị và thu hồi quyền Google từ tài khoản Google. Để yêu cầu truy cập, chỉnh sửa hoặc xóa dữ liệu tài khoản, hãy dùng kênh hỗ trợ chính thức được công bố trên Mingly."] },
  ...
  en: {
    ...
    notice: "Mingly does not record rooms. Mingly currently provides no Spotify connection or music-service OAuth. You control microphone, camera, screen sharing, and the YouTube player through your browser.",
    sections: [
...
=======
      { title: "Third-party services and embedded content", paragraphs: ["Mingly uses authentication, storage, and real-time communications providers to operate the service. YouTube is used only when you choose to search, add, or play content; its embedded player may apply YouTube terms and technologies.", "We do not sell personal data or expose sign-in tokens to people in your room or their browsers."] },
      { title: "Retention, security, and your choices", paragraphs: ["We retain room data and messages for as long as needed to operate, secure, and address legitimate requests. We enforce server-side access controls and do not expose room passwords, sign-in tokens, or service keys to other users.", "Mingly does not collect or store Spotify tokens or music-service OAuth tokens. You can leave a room, disable device permissions, and revoke Google access from your Google Account. To request access, correction, or deletion of account data, use the official support channel published on Mingly."] },
