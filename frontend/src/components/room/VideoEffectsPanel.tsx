...
export function VideoEffectsPanel({ cameraEnabled, triggerRef }: { cameraEnabled: boolean; triggerRef?: React.RefObject<HTMLButtonElement | null> }) {
  ...
  return (
    <div role="dialog" aria-label={tr("Video effects")} className="fixed sm:absolute left-2 right-2 bottom-[calc(5rem+env(safe-area-inset-bottom))] sm:left-1/2 sm:right-auto sm:bottom-14 sm:-translate-x-1/2 z-50 sm:w-[30rem] max-h-[min(70vh,38rem)] overflow-y-auto rounded-[6px] border border-[var(--border-loft)] bg-[var(--bg-loft-card)] shadow-2xl p-4">
      <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2 font-medium"><Sparkles className="w-4 h-4 text-[var(--text-loft-primary)]" />{tr("Video effects")}</div><button className="w-8 h-8 rounded-full hover-invert hover:bg-[var(--border-loft)] hover:text-[var(--bg-loft-base)] flex items-center justify-center" aria-label={tr("Close")} onClick={() => { setPanelOpen(false); triggerRef?.current?.focus(); }}><X className="w-4 h-4" /></button></div>
      {status && <p role="status" className="text-[11px] text-[var(--text-loft-secondary)] mb-3 flex gap-2 items-center">{runtime.status === "loading" && <LoaderCircle className="w-3.5 h-3.5 animate-spin" />}{status}</p>}
      ...
      <div className="mt-2"><input ref={fileRef} hidden type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void chooseImage(event.target.files?.[0])} /><button disabled={!cameraEnabled || !capabilities.segmentation} aria-pressed={selection.background === "custom"} onClick={() => fileRef.current?.click()} className={`w-full rounded-[6px] border px-3 py-2 text-[11px] flex items-center justify-center gap-2 disabled:opacity-40 ${selection.background === "custom" ? "border-[var(--border-loft)] bg-[#101113]/15" : "border-[var(--border-loft)]"}`}><ImagePlus className="w-4 h-4" />{tr("Custom background")}</button>{imageError && <p role="alert" className="mt-2 text-[11px] text-[var(--text-loft-primary)]">{imageError}</p>}</div>
...
function EffectGroup({ label, value, options, disabled, onChange }: { label: string; value: string; options: ReadonlyArray<{ id: string; label: string }>; disabled: boolean; onChange: (value: string) => void }) {
  const tr = useUIText();
  return <fieldset className="mt-4"><legend className="text-[11px] uppercase tracking-wider text-[var(--text-loft-secondary)] mb-2">{label}</legend><div role="radiogroup" className="grid grid-cols-3 gap-2">{options.map((option) => <button type="button" role="radio" aria-checked={value === option.id} disabled={disabled} key={option.id} onClick={() => onChange(option.id)} className={`min-h-10 rounded-[6px] border px-2 py-2 text-[11px] disabled:opacity-40 ${value === option.id ? "border-[var(--border-loft)] bg-[#101113]/15 text-[var(--text-loft-primary)]" : "border-[var(--border-loft)] hover-invert hover:bg-[var(--border-loft)] hover:text-[var(--bg-loft-base)]"}`}>{tr(option.label)}</button>)}</div></fieldset>;
}
