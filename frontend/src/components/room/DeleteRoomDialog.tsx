...
export function DeleteRoomDialog({ room, locale, pending, error, onCancel, onConfirm }: {
  ...
  return (
    <div className="fixed inset-0 bg-[#101113]/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" role="presentation">
      <div role="alertdialog" aria-modal="true" aria-labelledby="delete-room-title" aria-describedby="delete-room-description" className="w-full max-w-md rounded-[6px] border border-[var(--border-loft)] bg-[var(--bg-loft-card)] p-6 shadow-2xl text-[var(--text-loft-primary)]">
        ...
        <div className="mt-6 flex justify-end gap-2">
          <button type="button" disabled={pending} onClick={onCancel} className="rounded-[6px] border border-[var(--border-loft)] px-4 py-2 text-[11px] disabled:opacity-50">{locale === "vi" ? "Hủy" : "Cancel"}</button>
          <button type="button" disabled={pending} onClick={onConfirm} className="rounded-[6px] bg-[#101113] px-4 py-2 text-[11px] font-medium text-white disabled:opacity-50">{pending ? (locale === "vi" ? "Đang xóa…" : "Deleting…") : (locale === "vi" ? "Xóa phòng" : "Delete room")}</button>
        </div>
      </div>
    </div>
  );
