# REST API Conventions & Error Standards — Mingly
...
- **Use HTTP REST for:**
  ...
  - Resolving public room IDs and invitation links.
  - Durable query reads such as recent chat messages.
...
- `POST /api/v1/rooms` — Create a new room (Requires Auth).
  - Body: `{ "name": "Friday Chill", "allow_guests": true, "password_enabled": false, "password": "" }`
...
- `PATCH /api/v1/rooms/{id}` — Update owner-controlled room settings.
  - Body: `{ "expected_version": 2, "name": "Friday Chill", "allow_guests": true, "password_enabled": true, "password": "secret", "locked": false }`
  - The version fence prevents overwriting a newer room state.
